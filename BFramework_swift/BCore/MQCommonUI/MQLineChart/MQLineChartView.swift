...
protocol MQLineChartViewDelegate:class {
    func mqLineChartView(_ lineChartView:MQLineChartView,selectedAt index:Int)
    func mqLineChartView(_ lineChartView:MQLineChartView,animation finished:Bool)
}

extension MQLineChartViewDelegate {
    func mqLineChartView(_ lineChartView:MQLineChartView,selectedAt index:Int){}
    func mqLineChartView(_ lineChartView:MQLineChartView,animation finished:Bool){}
}

protocol MQLineChartViewDataSource:class {
    func numberOfValues(in lineChartView:MQLineChartView) -> Int
    func mqLineChartView(_ lineChartView:MQLineChartView,titleAt index:Int) -> String
    func mqLineChartView(_ lineChartView:MQLineChartView,valueAt index:Int) -> NSNumber
    /*起始点 默认： 0 0*/
    func preValue(in lineChartView:MQLineChartView) -> NSNumber?
    /*终止点 默认：根据values中最后两点趋势计算*/
    func lastValue(in lineChartView:MQLineChartView) -> NSNumber?
}

extension MQLineChartViewDataSource {
    func preValue(in lineChartView:MQLineChartView) -> NSNumber? {
        return nil
    }
...
class MQLineChartView: UIView {
    weak var delegate:MQLineChartViewDelegate?
    weak var dataSource:MQLineChartViewDataSource?
    ...
    fileprivate var lineMargin:CGFloat {//起止点 距屏幕边距
        get {
            if values.count > 0 {
                return MQLineChartConstant.width / CGFloat(values.count) / 2
            }
            return 30
        }
    }
    fileprivate var spaceRatio:CGFloat {//前后两点间距 与中间值间距不相等，算出的 前后两点Y坐标 需要调整
        get {
            if values.count > 0 {
                return lineMargin / (MQLineChartConstant.width / CGFloat(values.count))
            }
            return 1.0
        }
    }
    ...
    fileprivate var maskLayerPointsValue:Array<NSValue> {//填充轨迹(包括左下角，右下角)
        get {
            ...
            points.insert(NSValue.init(cgPoint: CGPoint(x: 0, y: MQLineChartConstant.chartHeight)), at: 0)
            points.append(NSValue.init(cgPoint: CGPoint(x: MQLineChartConstant.width, y: MQLineChartConstant.chartHeight)))
            return points
        }
    }
    //点击点时，弹出显示值
    fileprivate var popView: UIView = {
        ...
        view.backgroundColor = MQLineChartConstant.lineColor
        return view
    }()
    ...
    init(_ origin:CGPoint) {
        values = []
        days = []
        super.init(frame: CGRect(x: origin.x, y: origin.y, width: MQLineChartConstant.width, height: MQLineChartConstant.height))
        self.backgroundColor = MQLineChartConstant.backgroundColor
    ...
    fileprivate var firstReload = true
    ...
    override func draw(_ rect: CGRect) {
        ...
        for i in 0..<4 {
            context?.move(to: CGPoint(x: 0, y: MQLineChartConstant.chartHeight / 4.0 * CGFloat(i) + 0.5))
            context?.addLine(to: CGPoint(x: MQLineChartConstant.width, y: MQLineChartConstant.chartHeight / 4.0 * CGFloat(i)))
        }
        context?.strokePath()
    ...
    fileprivate var chartContentLayer: CALayer = {
        let layer = CALayer()
        layer.frame = CGRect(x: 0, y: 0, width: MQLineChartConstant.width, height: MQLineChartConstant.height)
    ...
    fileprivate var chartFillColorLayer: CALayer = {
        let layer = CALayer()
        layer.frame = CGRect(x: 0, y: 0, width: MQLineChartConstant.width, height: MQLineChartConstant.height)
        layer.backgroundColor = MQLineChartConstant.maskColor.cgColor
        return layer
    }()
    ...
    fileprivate var lineLayer: CAShapeLayer = {
        ...
        layer.lineWidth = 1.0
        layer.strokeColor = MQLineChartConstant.lineColor.cgColor
    ...
    fileprivate var animationLayer: CAShapeLayer = {
        let layer = CAShapeLayer()
        layer.path = UIBezierPath.init(rect: CGRect(x: 0, y: 0, width: MQLineChartConstant.width, height: MQLineChartConstant.height)).cgPath
        layer.position = CGPoint(x: -MQLineChartConstant.width, y: 0)
        return layer
    }()
    ...
    fileprivate var dotLayers:Array<CAShapeLayer> {
        get{
            ...
            for point in pointValues {
                ...
                dotLayer.strokeColor = MQLineChartConstant.lineColor.cgColor
...
extension MQLineChartView {
    fileprivate func buildYLabels() {
        ...
        for count in (1..<5).reversed() {
            let label = UILabel.init(frame: CGRect(x: 14, y: (MQLineChartConstant.chartHeight / 4) * CGFloat(4 - count), width: 60, height: 22))
    ...
    fileprivate func buildXLables() {
        ...
        if days.count > 0 {
            let margin = 10
            let width = (MQLineChartConstant.width - CGFloat(days.count * margin)) / CGFloat(days.count)
            for i in 0..<days.count {
                let label = UILabel.init(frame: CGRect(x: 5 + (width + CGFloat(margin)) * CGFloat(i) , y: MQLineChartConstant.chartHeight, width: width, height: 40))
                label.font = UIFont.systemFont(ofSize: 13)
                label.adjustsFontSizeToFitWidth = true
                label.textAlignment = .center
                label.textColor = UIColor.lightGray
                label.highlightedTextColor = MQLineChartConstant.lineColor
                label.text = days[i]
                self.addSubview(label)
    ...
    func reloadData() {
        ...
        self.dotsContentLayer.removeAnimation(forKey: MQLineChartConstant.dotsAnimationKey)
        self.animationLayer.removeAnimation(forKey: MQLineChartConstant.lineAnimationKey)
        //Remove PopView
        ...
        self.animationLayer.position = CGPoint(x: -MQLineChartConstant.width, y: 0)
        ...
        if let dataSource = dataSource {
            ...
            if count > 0 {
                ...
                for i in 0..<count {
                    values.append(dataSource.mqLineChartView(self, valueAt: i))
                    days.append(dataSource.mqLineChartView(self, titleAt: i))
                }
                //ReBuild
                buildYLabels()
                buildLineLayer()
...
extension MQLineChartView {
    func runLine_FillLayerAnimation() {
        ...
        animation.fromValue = NSValue.init(cgPoint: CGPoint(x: -MQLineChartConstant.width, y: 0))
        animation.toValue   = NSValue.init(cgPoint: CGPoint(x: 0, y: 0))
        animation.duration = 0.75
        animation.fillMode = kCAFillModeForwards
        animation.isRemovedOnCompletion = false
        self.animationLayer.add(animation, forKey: MQLineChartConstant.lineAnimationKey)
    ...
    func runDotsAnimation() {
        ...
        animation.fillMode = kCAFillModeForwards
        animation.isRemovedOnCompletion = false
        self.dotsContentLayer.add(animation, forKey: MQLineChartConstant.dotsAnimationKey)
    ...
    func runPopOverViewAnimation() {
        ...
        delegate?.mqLineChartView(self, animation: true)
    }
...
extension MQLineChartView {
    ...
    func pointValue(_ num:NSNumber,index:Int) -> CGPoint {
        ...
        var x = lineMargin
        if index != 0 {
            x += CGFloat(index) * (MQLineChartConstant.width - 2 * lineMargin) / CGFloat(values.count - 1)
        }
        var y:CGFloat = 0
        if max <= 1 {
            if floatMaxY > 0 {
                let tempF = self.maxYLess1FloatValue(floatMaxY)
                y = MQLineChartConstant.chartHeight * CGFloat(1 - num.floatValue / tempF)
            }else {
                y = MQLineChartConstant.chartHeight * CGFloat(1 - num.floatValue / 1.0)
            }
        } else {
            y = MQLineChartConstant.chartHeight * CGFloat(1 - num.floatValue / Float(max))
        }
    ...
    func prePoint() -> NSValue {
        if let preV = preValue {
            ...
            if max <= 1 {
                if floatMaxY > 0 {
                    let tempF = self.maxYLess1FloatValue(floatMaxY)
                    y = MQLineChartConstant.chartHeight * CGFloat(1 - preV.floatValue / tempF)
                }else {
                    y = MQLineChartConstant.chartHeight * CGFloat(1 - preV.floatValue / 1.0)
                }
            } else {
                y = MQLineChartConstant.chartHeight * CGFloat(1 - preV.floatValue / Float(max))
            }
            //有确定的值 不做调整
            //y *= spaceRatio
            //if y > MQLineChartConstant.chartHeight {
            //    y = MQLineChartConstant.chartHeight
            //}
            
            return NSValue.init(cgPoint: CGPoint(x: 0, y: y))
        }
        return NSValue.init(cgPoint: CGPoint(x: 0, y: MQLineChartConstant.chartHeight))
    }
    //values 之外的终点坐标
    func lastPoint() -> NSValue {
        if let lastV = lastValue {
            ...
            if max <= 1 {
                if floatMaxY > 0 {
                    let tempF = self.maxYLess1FloatValue(floatMaxY)
                    y = MQLineChartConstant.chartHeight * CGFloat(1 - lastV.floatValue / tempF)
                }else {
                    y = MQLineChartConstant.chartHeight * CGFloat(1 - lastV.floatValue / 1.0)
                }
            } else {
                y = MQLineChartConstant.chartHeight * CGFloat(1 - lastV.floatValue / Float(max))
            }
            //有确定的值 不做调整
            //y *= spaceRatio
            //if y > MQLineChartConstant.chartHeight {
            //    y = MQLineChartConstant.chartHeight
            //}
            return NSValue.init(cgPoint: CGPoint(x: MQLineChartConstant.width, y: y))
        }else {
            let count = pointValues.count
            if count > 0 {
                if count == 1 {//只有一个点
                    var pnt = (pointValues.first?.cgPointValue) ?? CGPoint.zero
                    pnt.x = MQLineChartConstant.width
                    pnt.y -= 2
                    ...
                    if pnt1.y < pnt2.y {//上升趋势变缓
                        ...
                        if pnt1.y > MQLineChartConstant.chartHeight {//避免底部绘图边界
                            pnt1.y = MQLineChartConstant.chartHeight
                        }
                    }
                    pnt1.x = MQLineChartConstant.width
                    return NSValue.init(cgPoint: pnt1)
                }
            }
            return NSValue.init(cgPoint: CGPoint(x: MQLineChartConstant.width, y: 0))
        }
    }
    //Y轴最大值
    func maxY() -> Int {
    ...
    func selectedPoint(at index:Int) {
        if values.count > 0 {
            ...
            if highLightedIndex > values.count - 1 {
                highLightedIndex = values.count - 1
            }
    ...
    func showPopLabel(at index:Int) {
        if values.count > 0 {
            ...
            let str = num.mq_priceString(false)
            ...
            } else if (point.x + textSize.width / 2.0) > MQ_BOUNDS_WIDTH { //右边超出边界
                offsetX = -fabs((MQ_BOUNDS_WIDTH - point.x) - textSize.width / 2.0)
            }
            ...
            if let touch = touches.first {
                ...
                if pointValues.count > 1 {
                    for i in 0..<(pointValues.count - 1) {
                        ...
                        var compareDistance = ((MQLineChartConstant.width - self.lineMargin * 2) / CGFloat(values.count) ) / 2
                        ...
                        if distance <= compareDistance { //点 周围 20pnt内点击有效
                            ...
                            delegate?.mqLineChartView(self, selectedAt: index)
                            break
                        }
                    }
                }else {
                    ...
                    if distance <= compareDistance { //点 周围 20pnt内点击有效
                        selectedPoint(at: 0)
                        delegate?.mqLineChartView(self, selectedAt: 0)
                    }
                }
            }
        }
