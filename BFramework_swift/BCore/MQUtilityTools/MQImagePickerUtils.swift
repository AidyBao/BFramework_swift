...
/// MQImagePickerUtils
class MQImagePickerUtils: NSObject,UIImagePickerControllerDelegate,UINavigationControllerDelegate {
    ...
    func takePhoto(presentFrom rootVC:UIViewController,completion:HCompletion?) {
        ...
        if MQImagePickerUtils.isCameraAvailable() && MQImagePickerUtils.doesCameraSupportTakingPhotos(){
            MQImagePickerUtils.cameraAuthorized { (authorized, status) in
    ...
    func choosePhoto(presentFrom rootVC:UIViewController,completion:HCompletion?){
        ...
        if MQImagePickerUtils.isPhotoLibraryAvailable(){
            MQImagePickerUtils.photoAuthorized({ (authorized, status) in
                if authorized || status == .notDetermined {
                    let controller = UIImagePickerController()
                    controller.view.backgroundColor = UIColor.white
                    controller.sourceType = UIImagePickerControllerSourceType.photoLibrary
                    var mediaTypes = [String]()
                    if MQImagePickerUtils.canUserPickPhotosFromPhotoLibrary(){
