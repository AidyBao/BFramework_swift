...
class ZXAddressViewController: UIViewController {
    ...
    //MARK: - UI
    func setUI() {
        self.cancelButton.titleLabel?.font = UIFont.mq_bodyFont
        self.cancelButton.setTitleColor(UIColor.mq_textColorBody, for: .normal)
        
        self.confirmButton.titleLabel?.font = UIFont.mq_bodyFont
        self.confirmButton.setTitleColor(UIColor.mq_tintColor, for: .normal)
        
        self.titleLB.textColor = UIColor.mq_textColorTitle
        self.titleLB.font = UIFont.mq_bodyFont
        self.titleLB.text = ""
    }
    ...
    func loadFirstData() {
        ...
        if list.count > 0 {
            ...
            MQHUD.showLoading(in: self.view, text: "", delay: 0)
            ...
            ZXLoginManager.requestForGetArea { (array) in
                ...
                if array.count > 0 {
                    ...
                    MQHUD.showFailure(in: self.view, text: "无法获取地址数据", delay: ZX.HUDDelay)
    ...
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        ...
        case 0:
            if self.provinceArray.count > 0 {
                ...
                title = provinceModel.shortName
            }
        case 1:
            if self.cityArray.count > 0 {
                let cityModel = self.cityArray[row]
                title = cityModel.shortName
            }
        case 2:
            if self.countyArray.count > 0 {
                let praishModel = self.countyArray[row]
                title = praishModel.shortName
            }
        default:
            break
        }
